            ...
            for col in meta['rows'][next(iter(meta['rows']))].keys():
                ...
                if st.session_state.get("advanced_mode", False):
                    if st.button(f"{col}", key=f"{table}_{col}"):
                        st.session_state.advanced_query_text += f'"{table}.{col}" '
                        st.rerun()
                else:
                    st.markdown(f"- {col}")
...
# ------------------------ MAIN PANEL HEADER ------------------------
col_main, col_adv = st.columns([5, 1])
with col_main:
...
if st.session_state.advanced_mode:
    ...
    if "advanced_query_text" not in st.session_state:
        st.session_state.advanced_query_text = """select_query(
...
"""
    ...
    query_input = st.text_area(
        ...
        value=st.session_state.advanced_query_text,
        height=400,
        key="adv_query_area"
    )
    ...
    if st.button("🚀 Run Query"):
        import traceback
        try:
            # ✅ Extract everything inside select_query(...)
            match = re.search(r"select_query\s*\(([\s\S]*)\)", query_input)
            if not match:
                st.error("⚠️ Could not find a valid `select_query(...)` call.")
                st.stop()

            inner_content = match.group(1).strip()

            # ✅ Convert function-style args (key=value) → dict-style ('key': value)
            inner_content = re.sub(r"(\w+)\s*=", r"'\1':", inner_content)

            # ✅ Now make a proper dictionary
            formatted_str = "{" + inner_content + "}"

            try:
                query_kwargs = ast.literal_eval(formatted_str)
            except Exception as parse_err:
                st.error("⚠️ Failed to parse query parameters. Check your syntax.")
                st.code(traceback.format_exc())
                st.stop()

            # ✅ Run the backend query
            result = db.select_query(**query_kwargs)

            if result is None:
                st.error("⚠️ `db.select_query()` returned None. Check your parameters or table names.")
                st.code(str(query_kwargs), language="python")
                st.stop()
    ...
    st.stop()  # ✅ Prevent showing normal builder below
...
with col1:
    ...
    if enable_join:
        ...
        # ✅ Inline Join Type
        col_label, col_radio = st.columns([0.15, 0.85])
        with col_label:
            st.markdown("**Join Type:**")
        with col_radio:
            join_type = st.radio("", ["inner", "left"], horizontal=True, key="join_type")
