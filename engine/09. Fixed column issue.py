...
"""
    ...
    if st.button("🚀 Run Query"):
        import traceback
        try:
            # ⚡ Run the user's select_query(...) directly
            result = eval(query_input, {"select_query": db.select_query})
            ...
            if not result:
                st.warning("⚠️ Query returned an empty result.")
                st.stop()
    
            # ✅ Create DataFrame using dict keys from result — preserves order
