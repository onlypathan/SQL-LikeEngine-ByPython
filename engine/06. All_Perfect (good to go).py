...
with col1:
    ...
    def _infer_type(value: str):
        """Return int, float, str or None depending on input text."""
    ...
    if enable_where:
        ...
        if "from_table" not in locals() or not from_table:
            ...
            # ✅ Ensure there's always at least one default condition row
            ...
            st.caption([
                f"{c['col']} {c['op']} {c['val']} ({type(c['val']).__name__})"
                for c in st.session_state.where_conditions
            ])
    else:
        st.session_state.where_conditions = []  # ✅ clear old conditions
 ...
 # ✅ COLUMNS SELECTION
    ...
    # ✅ RUN QUERY
# ✅ RUN QUERY BUTTON (should be outside forms)
try:
    if st.button("Run Query"):
        joins = [(join_table, (left_key, right_key), join_type)] if enable_join else None
    
        # Convert session_state WHERE into proper format
        where = []
        for i, cond in enumerate(st.session_state.where_conditions):
