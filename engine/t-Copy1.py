...
with col1:
    ...
    if not enable_where:
        ...
        # ✅ Wrap WHERE inputs inside a form
        with st.form("where_form"):
            ...
            for i, cond in enumerate(st.session_state.where_conditions):
                ...
                op = cols[2].selectbox(
                    "Operator", ["=", "!=", ">", "<", ">=", "<="],
                    key=f"op_{i}", index=["=", "!=", ">", "<", ">=", "<="].index(cond["op"])
                )
    
                # Value
                val_input = cols[3].text_input("Value", value=cond["val"], key=f"val_{i}")
            ...
            # ➕ Add new WHERE condition button
            button_cols = st.columns([8, 2])  # adjust 8,2 → moves button further right
            add_clicked = st.form_submit_button("Add WHERE Condition")
            if add_clicked:
                new_conditions.append({
                    "logic": "AND",
                    "col": all_columns[0],
                ...
                st.rerun()  # ✅ Correct for Streamlit >= 1.31
    
        # ✅ Save updated WHERE conditions (even without add click)
        st.session_state.where_conditions = new_conditions
